...
let activeConn = null;
  ...
  */ 
 const {initAuthCreds, BufferJSON, proto} = (await import('@whiskeysocket/baileys')).default; 
  
 /** 
  * @param {import('@whiskeysocket/baileys').WASocket | import('@whiskeysocket/baileys').WALegacySocket} 
  */ 
 function bind(conn) { 
  activeConn = conn; 
 ...
 function loadMessage(jid, id = null) { 
   ...
   if (targetJid && !targetId) { 
     ...
     for (const chat of Object.values(conn.chats)) { 
       const message = chat?.messages?.[targetId]; 
       if (message) return message; 
     } 
     return null; 
   } 

   targetJid = typeof targetJid === 'string' ? conn.decodeJid(targetJid) : targetJid; 
   const chat = targetJid ? conn.chats[targetJid] : null; 
   if (!chat?.messages) return null; 
   return chat.messages[targetId] || null; 
 } 
 export default { 
   bind, 
