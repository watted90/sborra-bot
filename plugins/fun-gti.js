...
async function applicaEffetto(m, conn, tipoEffetto, target) {
...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    const tipoEffetto = command.toLowerCase().replace(/ /g, '');
    ...
    await applicaEffetto(m, conn, tipoEffetto, target);
}

handler.help = ['gay @user', 'trans', 'sborra', 'ilove <nome>', 'il @nome'];
handler.tags = ['fun', 'sticker'];
handler.command = /^(gay|trans|sborra|ilove|il|i ?love)$/i;
